...
describe("allPairs()", () => {
  ...
  test("should return correct result", () => {
    ...
    expect(actual3).toStrictEqual(new Vec());

    const vec4 = new Vec();
    const other4 = new Vec(1, 2);
    const actual4 = Vec.allPairs(vec4, other4);
    expect(actual4).toStrictEqual(new Vec());

    const vec5 = Vec.of(1);
    const other5 = Vec.of(2);
    const actual5 = Vec.allPairs(vec5, other5);
    expect(actual5).toStrictEqual(new Vec(new Vec(1, 2)));

    expect(Vec.allPairs(Vec.from([, 1, ,]), [, 0])).toStrictEqual(
      new Vec(
        Vec.from([, ,]),
        Vec.from([, 0]),
        Vec.from([1, ,]),
